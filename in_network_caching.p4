...

{
    ...
    DirectCounter<bit<64>>(CounterType_t.PACKETS_AND_BYTES) filesize_stats;    
    ...
    table nexthop {
     ...
     /********* MIRRORING ************/
    action acl_mirror(bit<32> filesize, MirrorId_t mirror_session) {
	...
	filesize_stats.count();
    }

    action acl_drop_and_mirror(bit<32> filesize, MirrorId_t mirror_session) {
        acl_mirror(filesize,mirror_session);
        drop();
    }

    table mirror_port_ekl {
        key = {
           hdr.ipv4.src_addr : exact;
        }
        actions = {
            acl_mirror; @defaultonly NoAction; 
		//acl_drop_and_mirror; drop; NoAction;
        }
        size = 512;
	counters = filesize_stats; 
